...
public class NGramMap {
    ...
    public IPriorityQueue.PQElement<String>[] getCountsAfter(NGram ngram) {
        IDictionary<IterableString, Integer> dope = this.map.get(ngram);
        if(dope == null){return null;}
        int smalls = 0;
        IPriorityQueue.PQElement<String>[] op = new IPriorityQueue.PQElement[dope.size()];
        for(IterableString str: dope.keys()){
            op[smalls] = new IPriorityQueue.PQElement<String>(str.toString(), dope.get(str));
            smalls++;
        }
        return op;
    }
    ...
    IPriorityQueue.PQElement<String>[] cnts = getCountsAfter(ngram);
    if(cnts == null) return new String[0];
    int rK = k;
    if(cnts.length < k) rK = cnts.length;
    TopKSort.sort(cnts, rK);
    String[] op = new String[rK];
    for(int i = 0; i < rK; i++){
        op[i] = cnts[i].data;
    }
    return op;
