...
public class SpellingCorrector {
    ...
    private static int helper(String a, String b, Integer[][] arr) {
        Integer check = arr[a.length()][b.length()];
        if(check != null) {
            return check;
        }
        if(a.equals(b)) {
            arr[a.length()][b.length()] = 0;
            return 0;
        }
        if(a.length() == 1 && b.length() == 1) return 1;
        if(a.equals("")) {
            arr[a.length()][b.length()] = b.length();
            return b.length();
        }
        if(b.equals("")) {
            arr[a.length()][b.length()] = a.length();
            return a.length();
        }
        else {
            ...
            if(a.charAt(a.length() - 1) == b.charAt(b.length() - 1)) {
                int dist = helper(substring1, substring, arr);
                arr[a.length()][b.length()] = dist;
                return dist;
            }
            else{
                int first = 1 + helper(substring1, b, arr);
                int second = 1 + helper(a, substring, arr);
                int third = 1 + helper(substring1, substring, arr);
                if(first <= second && first <= third) {
                    arr[a.length()][b.length()] = first;
                    return first;
                }
                if(second <= first && second <= third){
                    arr[a.length()][b.length()] = second;
                    return second;
                }
                else {
                    arr[a.length()][b.length()] = third;
                    return third;
                }
            }
        }
