...
public class Maze {
    ...
    public Point[] getChildren(Point point) {
        int size = 0;
        Point[] childs = new Point[4];
        if(!north[point.x][point.y]) {
            Point urmom  = new Point(point.x, point.y + 1);
            urmom.parent = point;
            childs[size] = urmom;
            size++;
        }
        ...
        if(!east[point.x][point.y]) {
            Point urmom  = new Point(point.x + 1, point.y);
            urmom.parent = point;
            childs[size] = urmom;
            size++;
        }
    ...
    private void solveDFSRecursive(Point point) {
            ...
            Point[] dawg = getChildren(point);
            for (int i = 0; i < dawg.length; i++) {
                if (!dawg[i].equals(point.parent)) {
                    solveDFSRecursive(dawg[i]);
                    if(done) return;
                }
            }
        }
    ...
    public void solveDFSIterative() {
        Stack<Point> daddy = new Stack<>();
        Point current = new Point(1, 1);
        selectPoint(current);
        while(!done) {
            for(Point child : getChildren(current)) {
                if(!child.equals(current.parent))
                daddy.push(child);
            }
            Point next = daddy.pop();
            selectPoint(next);
            current = next;
